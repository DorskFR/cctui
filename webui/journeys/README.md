...
fixture in `deploy/local/fixture/seed.sh`, logs in with the admin token from `CCTUI_TOKEN` or
`deploy/local/.env` (generated by `make local/up`), and lets a
...
- Each of the four onboarding journeys runs end to end in `guide` mode on a
  fresh instance with the local admin token and no fixture, in the order of section 3,
