...
async function openDispatch(prefill: Record<string, string> | null = null) {
  ...
  must(
    segments.find((b) => b.textContent?.trim().startsWith("Dispatch")),
    "dispatch segment",
  ).click();
  await tick(60);
}
...
async function addEnvRow(key: string, value: string) {
  ...
  add.click();
  await tick();
  const keyEl = document.querySelector<HTMLInputElement>(
    'input[aria-label="Secret name"]',
  );
  const valEl = document.querySelector<HTMLInputElement>(
...
describe("SpawnModal context pack", () => {
  ...
  it("adds nothing when the pack fields are empty", async () => {
    ...
    expect(dispatch).toHaveBeenCalledTimes(1);
    expect(
      (dispatch.mock.calls[0][0].payload as { env?: unknown }).env,
    ).toBeUndefined();
  });
  ...
  it("never writes the pack token to the local draft slot", async () => {
    ...
    const slot = JSON.parse(
      localStorage.getItem(localStorage.getItem("cctui_spawn_slot")!)!,
    );
