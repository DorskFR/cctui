...
export class CacheColdClock {
	...
	ttlMs = $derived(cacheTtlMs(this.#o.adapterId(), this.#o.model()));
	...
	#slowNow = $derived(this.#clock(15_000));
	#nearCold = $derived.by(() => {
	...
	now = $derived(this.#nearCold ? this.#clock(1_000) : this.#slowNow);
	cold = $derived(
		!this.#o.working() && this.lastActivityMs !== null && this.now - this.lastActivityMs > this.ttlMs
	);
	msUntilCold = $derived(
		this.lastActivityMs === null ? null : this.ttlMs - (this.now - this.lastActivityMs)
	);
	imminent = $derived(
		!this.#o.working() && this.msUntilCold !== null && this.msUntilCold > 0 && this.msUntilCold <= COLD_WARN_MS
	);
	countdownSecs = $derived(this.imminent ? Math.ceil((this.msUntilCold as number) / 1000) : null);
}
