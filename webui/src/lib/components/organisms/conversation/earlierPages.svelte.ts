...
export interface EarlierPagesOpts {
	...
	events: () => AgentEvent[];
	fetch?: (id: string, q: { limit: number; before: number }) => Promise<AgentEvent[]>;
}
...
export class EarlierPages {
	...
	canFetch = $derived(
		!this.exhausted && (this.#o.historyLength() >= CONVERSATION_FETCH_LIMIT || this.pages.length > 0)
	);
	...
	fetchEarlier = async (): Promise<void> => {
		...
		this.fetching = true;
		try {
			const page = await this.#fetch(sid, { limit: CONVERSATION_FETCH_LIMIT, before: oldest });
