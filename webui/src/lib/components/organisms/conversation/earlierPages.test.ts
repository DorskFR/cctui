...
const ev = (seq: number) => ({ seq, ts: seq, type: 'text' }) as unknown as AgentEvent;
const page = (from: number, n: number) => Array.from({ length: n }, (_, i) => ev(from + i));

function make(over: Partial<{ id: string; history: number; events: AgentEvent[] }> = {}) {
	let id = over.id ?? 's1';
	const fetch = vi.fn(async (_sid: string, q: { limit: number; before: number }) =>
		page(q.before - q.limit, q.limit)
	);
	const pages = new EarlierPages({
		id: () => id,
...
describe('EarlierPages', () => {
	...
	it('prepends a full page before the oldest known seq and keeps offering more', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('s1', { limit: CONVERSATION_FETCH_LIMIT, before: 500 });
