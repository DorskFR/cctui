// A small, hand-picked emoji catalogue for the account glyph picker. Desktop
// browsers have no emoji keyboard, so the editor offers this grid instead of
// asking the user to paste one. Keywords are English + French so the search
// box works in both interface languages; no external dataset, no network.
...
export function searchEmoji(query: string, groups: readonly EmojiGroup[] = EMOJI_GROUPS): EmojiEntry[] {
