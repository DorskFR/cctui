<script lang="ts">
	...
	import {
		...
		type BatteryBars,
		type BatteryEntry
	} from '$lib/components/molecules/UsageBattery.logic';

	// The header strip: one battery per provider with usage, grouped by
	// account; below 640px a single worst-headroom battery stands in for all
	// of them. Renders nothing when no account reports usage or the user hid it.
	const q = useAllAccountsUsage(() => settings.usageBatteries);
	const entries = $derived(settings.usageBatteries ? batteryEntries(q.data) : []);
...
{#snippet cell(bars: BatteryBars, title: string, provider: string | null)}
	...
	<span class="cell" {title}>
		...
		{#if p}
			<span class="pace" data-state={p.state} aria-hidden="true">{p.glyph}{#if p.wall}<span class="wall">{p.wall}</span>{/if}</span>
		{/if}
	</span>
{/snippet}
