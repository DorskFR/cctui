...
/** One headline percentage for a provider: its 5h window, else the fullest
 *  window it reported, so a provider without a 5h window still reads. */
export function headlinePct(windows: UsageWindow[]): number | null {
	const session = windows.find((w) => w.key === 'session');
	if (session) return barPct(session);
...
/** A battery per pinned provider with usage. Pinning is the only control:
 *  unpin every provider and the strip disappears. */
