// Session-label color model. Labels carry an HSL *hue* (0–360), reusing the
// same hue infrastructure as MachineBadge / Swatch atoms — no hardcoded hex.
// The hue is persisted in the label's opaque `color` string (the server never
// parses it); an unset/unparseable value falls back to a deterministic name
// hash, exactly like an "Auto" machine color.

import { hashHue } from '$lib/format';

/** Preset hues offered for label colors, matching the per-machine palette.
 *  `null` (Auto) → deterministic name hash. */
export const LABEL_HUES = [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330];

/** The hue explicitly stored on a label, or `null` when unset ("Auto"). Used
 *  as the picker `value` so the Auto swatch reads as selected when unset. */
