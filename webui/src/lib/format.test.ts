...
describe('modelAbbrev', () => {
	it('abbreviates the Claude families', () => {
		...
		expect(modelAbbrev('claude-sonnet-5')).toBe('Son.');
	...
	it('tells the Codex codenames apart, where the family word cannot', () => {
		...
		expect(modelAbbrev('gpt-5.6-sol')).toBe('Sol.');
		expect(modelAbbrev('gpt-5.6-terra')).toBe('Te.');
		expect(modelAbbrev('gpt-5.6-luna')).toBe('Lu.');
		expect(modelAbbrev('gpt-6-astra')).toBe('As.');
	});

	it('spends a third letter only where two would collide', () => {
		expect(modelAbbrev('claude-sonnet-5')).not.toBe(modelAbbrev('gpt-5.6-sol'));
		for (const id of ['claude-opus-4-8', 'claude-fable-5-1', 'gpt-5.6-terra', 'gpt-6-astra']) {
			expect(modelAbbrev(id)).toHaveLength(3);
		}
	});
