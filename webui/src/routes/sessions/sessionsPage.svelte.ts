...
export interface SessionsPageDeps {
  ...
  api: Pick<
    typeof endpoints,
    "session" | "searchSessions" | "brief" | "searchFieldValues"
  >;
  actions: Pick<
    SessionActions,
    | "archive"
...
export class SessionsPage {
  ...
  constructor(d: SessionsPageDeps) {
    ...
    this.searchSchema = buildSessionSearchSchema((field, q) =>
      d.api.searchFieldValues(
        field,
        q,
        contextForField(this.rawQuery, this.searchSchema, field),
      ),
    );
    this.archiveConfirm = new ArchiveConfirm(this.runArchive, (e) =>
      d.toasts.error(errMessage(e)),
    );
    ...
    $effect(() => {
      ...
      if ([...this.labelFilter].some((id) => !known.has(id))) {
        this.labelFilter = new Set(
          [...this.labelFilter].filter((id) => known.has(id)),
        );
      }
    });
    $effect(() => {
  ...
  colorBy = $derived.by(
    () => this.#d.settings.state.sessionList.colorBy as Dimension,
  );
  groupBy = $derived.by(
    () => this.#d.settings.state.sessionList.groupBy as Dimension,
  );
  showMachine = $derived(this.groupBy !== "machine");
  sortState = $derived.by(() => ({
  ...
  docks = $derived.by(() => this.#d.dockLayout());
  dockSide = $derived(this.docks.spawn);

  items = $derived.by(() => this.#d.sessions());
  sessionsLoading = $derived.by(() => this.#d.sessionsLoading());
  allLabels = $derived.by(() => this.#d.labels() ?? []);
  ...
  #split = $derived.by(() => splitQuery(this.query, this.searchSchema));
  ...
  searchTerms = $derived.by(() =>
    tokenizeQuery(freeText(parse(this.query, this.searchSchema))),
  );
  pagerKey = $derived(
    `${this.searching ? `q:${this.serverQuery}` : "browse"}|${this.showArchived}`,
  );

  archiving = $derived.by(() => this.archivingOne || this.archiveConfirm.busy);
  ...
  pinnedIds = $derived(
    new Set(this.items.filter((s) => s.pinned).map((s) => s.id)),
  );
  #archivedPool = $derived.by(() =>
    this.#d.allSessions().filter((s) => s.status === "archived"),
  );
  pinnedArchivedKids = $derived(
    archivedDescendantsOf(this.pinnedIds, this.#archivedPool),
  );
  // Their ids, so the Archived browse doesn't also list them as their own
  // top-level rows.
  pinnedArchivedKidIds = $derived(
    new Set(this.pinnedArchivedKids.map((s) => s.id)),
  );
  childGroupsOf = $derived.by(() => this.list.childGroupsOf);
  ...
  focusSeq = $derived(
    this.focusHit && this.focusHit.id === this.liveOpen?.id
      ? this.focusHit.seq
      : null,
  );

  accentOf = (s: SessionListItem) => colorHueOf(s, this.colorBy);
  selectSort = (sort: SessionSort) =>
    this.#d.settings.setSessionList(nextSort(this.sortState, sort));
  ...
  matchesLabelFilter = (s: SessionListItem): boolean =>
    this.labelFilter.size === 0 ||
    s.labels.some((l) => this.labelFilter.has(l.id));
  matchesClient = (s: SessionListItem): boolean =>
    matchesClientFilters(s, this.#clientFilters);
  ...
  createLabel = (name: string, color: string) =>
    this.#d.actions.createLabel(name, color);
  attachLabel = (id: string, labelId: string) =>
    this.#d.actions.attachLabel(id, labelId);
  detachLabel = (id: string, labelId: string) =>
    this.#d.actions.detachLabel(id, labelId);
  ...
  openById = async (id: string) => {
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 404) {
        ...
        this.#d.toasts.error(
          m.sessions_toast_open_failed({ error: errMessage(e) }),
        );
      }
    } finally {
  ...
  archiveSection = (label: string, ids: string[]) => {
    this.archiveConfirm.request({
      ...
      message: m.sessions_confirm_archive_section({
        count: ids.length,
        section: label,
      }),
      ids,
  ...
  swipeArchive = async (s: SessionListItem) => {
    ...
    try {
      ...
      this.#d.toasts.ok(
        isArchived
          ? m.sessions_toast_unarchived()
          : m.sessions_toast_archived_one(),
        undefined,
  ...
  togglePin = async (s: SessionListItem) => {
    try {
      ...
      this.#d.toasts.ok(
        s.pinned ? m.sessions_toast_unpinned() : m.sessions_toast_pinned(),
      );
      this.refreshTick++;
    } catch (e) {
      this.#d.toasts.error(errMessage(e));
  ...
  launchDraft = async (s: SessionListItem) => {
    ...
    try {
      ...
      this.#d.toasts.error(
        m.sessions_toast_launch_failed({ error: errMessage(e) }),
      );
    } finally {
  ...
  editDraft = (s: SessionListItem) => {
    ...
    const live = form
      ? { dirty: form.isDirty(), draftId: form.currentDraftId() }
      : null;
    if (
      editDraftNeedsConfirm(
        s.id,
        live,
        this.#d.spawnSlot.read(this.#d.spawnSlot.currentKey()),
      )
    ) {
  ...
  #saveCurrentSpawnForm = async () => {
    const form = this.spawnModal;
    if (form) {
      if (!(await form.flushDraft()))
        throw new Error(m.spawn_draft_incomplete());
      return;
    }
  ...
  confirmDraftEdit = async (saveFirst: boolean) => {
    ...
    if (saveFirst) {
      try {
        ...
        this.#d.toasts.error(
          m.sessions_toast_save_current_failed({ error: errMessage(e) }),
        );
        return;
      }
    }
