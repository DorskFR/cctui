...
use super::wrapping::{RtOptions, adaptive_wrap_line};
use dirs::home_dir;
    ...
    I: Iterator<Item = Event<'a>>,
{
    ...
    fn start_blockquote(&mut self) {
        ...
        self.indent_stack.push(IndentContext::new(vec![Span::from("> ")], None, false));
    }
    ...
    fn text(&mut self, text: CowStr<'a>) {
        ...
        for (i, line) in text.lines().enumerate() {
            ...
            let span =
                Span::styled(content, self.inline_styles.last().copied().unwrap_or_default());
            self.push_span(span);
        }
        self.needs_newline = false;
    }
    ...
    fn start_codeblock(&mut self, lang: Option<String>, indent: Option<Span<'static>>) {
        ...
        self.indent_stack.push(IndentContext::new(vec![indent.unwrap_or_default()], None, false));
        self.needs_newline = true;
    }
    ...
    fn pop_link(&mut self) {
        if let Some(link) = self.link.take() {
            if link.show_destination {
                ...
                let style =
                    self.inline_styles.last().copied().unwrap_or_default().patch(self.styles.code);
...
fn normalize_markdown_hash_location_suffix(_s: &str) -> Option<String> {
    None
}
