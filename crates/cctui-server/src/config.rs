...
#[derive(Debug, Clone)]
pub struct Config {
    /// Env seeds for the default `CctuiAgent` limits (`CCTUI_SPAWN_MAX_CHILDREN`,
    /// `CCTUI_SPAWN_MAX_DEPTH`, `CCTUI_SPAWN_MAX_TREE_BUDGET_USD`); `None` when
    /// unset. A value saved in Settings wins over these.
    pub spawn_max_children: Option<u32>,
    pub spawn_max_depth: Option<u32>,
    pub spawn_max_tree_budget_usd: Option<f64>,
    pub host: String,
    pub port: u16,
...
impl Config {
    ...
    fn from_lookup(get: impl Fn(&str) -> Option<String>) -> anyhow::Result<Self> {
        ...
        let claude_litellm_models = set("CCTUI_CLAUDE_LITELLM_MODELS")
            ...
            .unwrap_or_default();
        let mut num = |k: &str| -> Option<f64> {
            set(k).and_then(|s| match s.trim().parse::<f64>() {
        ...
        let spawn_max_children = num("CCTUI_SPAWN_MAX_CHILDREN");
        let spawn_max_depth = num("CCTUI_SPAWN_MAX_DEPTH");
        let spawn_max_tree_budget_usd = num("CCTUI_SPAWN_MAX_TREE_BUDGET_USD");
        for (k, v) in [
            ("CCTUI_SPAWN_MAX_CHILDREN", spawn_max_children),
            ("CCTUI_SPAWN_MAX_DEPTH", spawn_max_depth),
        ] {
            if v.is_some_and(|v| v.fract() != 0.0 || v > f64::from(u32::MAX)) {
...
impl Config {
    ...
    pub fn for_test(allowed_origins: Vec<String>) -> Self {
        Self {
            spawn_max_children: None,
            spawn_max_depth: None,
            spawn_max_tree_budget_usd: None,
            host: "0.0.0.0".into(),
            port: 8700,
...
mod tests {
    ...
    fn spawn_defaults_are_env_tunable() {
        ...
        let cfg = Config::from_lookup(base).unwrap();
        assert_eq!(cfg.spawn_max_children, None);
        assert_eq!(cfg.spawn_max_depth, None);
        assert_eq!(cfg.spawn_max_tree_budget_usd, None);
        ...
        let cfg = Config::from_lookup(tuned).unwrap();
        assert_eq!(cfg.spawn_max_children, Some(4));
        assert_eq!(cfg.spawn_max_depth, Some(1));
        assert_eq!(cfg.spawn_max_tree_budget_usd, Some(12.5));
