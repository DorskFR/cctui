...
impl ServerClient {
    ...
    /// `kind='apple'`). The server mints an id + key the same way as the machine
    /// enroll; the key is persisted to `dispatcher.toml`.
    pub async fn enroll(&self, user_token: &str, name: &str) -> anyhow::Result<EnrollResponse> {
        ...
        let resp = self
            ...
            .json(&EnrollBody { name, kind: "apple" })
            .send()
            .await?;
...
mod tests {
    ...
    fn ws_url_upgrades_scheme_and_carries_token() {
        let c = ServerClient::new("https://cctui.example.test/");
        assert_eq!(
            c.dispatcher_ws_url("k-123"),
            "wss://cctui.example.test/api/v1/dispatcher/ws?token=k-123"
        );
        let c = ServerClient::new("http://localhost:8700");
