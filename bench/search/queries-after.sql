...
EXPLAIN (ANALYZE, BUFFERS) :sel WHERE (TRUE) AND ((s.id ILIKE '%error%' OR COALESCE(s.session_name, '') ILIKE '%error%' OR s.working_dir ILIKE '%error%' OR EXISTS (SELECT 1 FROM stream_events e WHERE e.session_id = (SELECT s.id) AND left(e.search_text, 8192) ILIKE '%error%'))) AND (NULL::uuid IS NULL OR m.user_id = NULL::uuid) ORDER BY s.registered_at DESC LIMIT 100 OFFSET 0;

\echo '=== B. main query: single RARE term (zygomorphic) ==='
EXPLAIN (ANALYZE, BUFFERS) :sel WHERE (TRUE) AND ((s.id ILIKE '%zygomorphic%' OR COALESCE(s.session_name, '') ILIKE '%zygomorphic%' OR s.working_dir ILIKE '%zygomorphic%' OR EXISTS (SELECT 1 FROM stream_events e WHERE e.session_id = (SELECT s.id) AND left(e.search_text, 8192) ILIKE '%zygomorphic%'))) AND (NULL::uuid IS NULL OR m.user_id = NULL::uuid) ORDER BY s.registered_at DESC LIMIT 100 OFFSET 0;

\echo '=== C. main query: TWO terms (quokka handler) -> two independent EXISTS ==='
EXPLAIN (ANALYZE, BUFFERS) :sel WHERE (TRUE) AND (((s.id ILIKE '%quokka%' OR COALESCE(s.session_name, '') ILIKE '%quokka%' OR s.working_dir ILIKE '%quokka%' OR EXISTS (SELECT 1 FROM stream_events e WHERE e.session_id = (SELECT s.id) AND left(e.search_text, 8192) ILIKE '%quokka%')) AND (s.id ILIKE '%handler%' OR COALESCE(s.session_name, '') ILIKE '%handler%' OR s.working_dir ILIKE '%handler%' OR EXISTS (SELECT 1 FROM stream_events e WHERE e.session_id = (SELECT s.id) AND left(e.search_text, 8192) ILIKE '%handler%')))) AND (NULL::uuid IS NULL OR m.user_id = NULL::uuid) ORDER BY s.registered_at DESC LIMIT 100 OFFSET 0;
...
EXPLAIN (ANALYZE, BUFFERS) :sel WHERE (TRUE) AND ((s.id ILIKE '%ok%' OR COALESCE(s.session_name, '') ILIKE '%ok%' OR s.working_dir ILIKE '%ok%' OR EXISTS (SELECT 1 FROM stream_events e WHERE e.session_id = (SELECT s.id) AND left(e.search_text, 8192) ILIKE '%ok%'))) AND (NULL::uuid IS NULL OR m.user_id = NULL::uuid) ORDER BY s.registered_at DESC LIMIT 100 OFFSET 0;
...
EXPLAIN (ANALYZE, BUFFERS) SELECT sid, hit.text, hit.id FROM unnest(ARRAY(SELECT id FROM sessions WHERE id LIKE 'bench-sess-%' ORDER BY registered_at DESC LIMIT 100)) AS sid CROSS JOIN LATERAL (SELECT left(e.search_text, 8192) AS text, e.id FROM stream_events e WHERE e.session_id = sid AND (left(e.search_text, 8192) ILIKE '%error%') ORDER BY e.created_at DESC LIMIT 1) hit;

\echo '=== J. SNIPPET pass at the 500-row limit ==='
EXPLAIN (ANALYZE, BUFFERS) SELECT sid, hit.text, hit.id FROM unnest(ARRAY(SELECT id FROM sessions WHERE id LIKE 'bench-sess-%' ORDER BY registered_at DESC LIMIT 500)) AS sid CROSS JOIN LATERAL (SELECT left(e.search_text, 8192) AS text, e.id FROM stream_events e WHERE e.session_id = sid AND (left(e.search_text, 8192) ILIKE '%error%') ORDER BY e.created_at DESC LIMIT 1) hit;
